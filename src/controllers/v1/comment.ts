...
export const createComment = async (req: Request, res: Response) => {
	...
	if (forum.locked) {
		...
		else if (!req.session.role?.includes("forum_moderator") && !req.session.role?.includes("admin")) return res.status(403).json({ data: null, message: "Forum is locked", success: false });
	}
